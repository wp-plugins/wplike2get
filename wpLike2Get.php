...
{
    wp_enqueue_style('wplike2get', WPLIKE2GET_URI . 'css/wplike2get.min.css', false, WPLIKE2GET_VERSION, 'all');
}
...
function wplike2get_enqueue_script()
{
    if (wplike2get_get_setting('fb_activated') || wplike2get_get_setting('tw_activated') || wplike2get_get_setting('gp_activated'))
        wp_enqueue_script('wplike2get-script', WPLIKE2GET_URI . 'js/wplike2get.min.js', array('jquery'), WPLIKE2GET_VERSION, true);
...
function wplike2get_shortcode($atts, $content = null)
{
    ...
    extract(shortcode_atts(array(
        ...
        "like_url" => "", // @since 1.2.8
        "facebook_url" => "", // @since 1.2.8
        "twitter_url" => "", // @since 1.2.8
        "gplusone_url" => "" // @since 1.2.8
    ), $atts));
    ...
    if ($show) {
        if (wplike2get_is_true($twitter) || wplike2get_is_true($facebook) || wplike2get_is_true($gplusone)) {
            ...
            if (wplike2get_get_setting('l2g_show_plugin_link')) {
                $return .= '<span class="l2g-plugin-link"><a href="http://markusdrubba.de/wordpress/wplike2get/">wpLike2Get</a></span>';
            }
            ...
            $fburl = $facebook_url ? ' data-url="' . $facebook_url . '"' : 'data-url="' . $like_url . '"';
            $twurl = $twitter_url ? ' data-url="' . $twitter_url . '"' : 'data-url="' . $like_url . '"';
            $gpurl = $gplusone_url ? ' data-url="' . $gplusone_url . '"' : 'data-url="' . $like_url . '"';

            $return .= '<div id="l2g" class="attachment-' . $id . '">';
            if (wplike2get_is_true($facebook)) {
                $return .= '<div class="facebook"' . $fburl . '></div>';
            }
            if (wplike2get_is_true($twitter)) {
                $return .= '<div class="twitter"' . $twurl . '></div>';
            }
            if (wplike2get_is_true($gplusone)) {
                $return .= '<div class="gplusone"' . $gpurl . '></div>';
            }
            $return .= '<br/>';
        }
        if (wplike2get_is_true($twitter) || wplike2get_is_true($facebook) || wplike2get_is_true($gplusone)) {
